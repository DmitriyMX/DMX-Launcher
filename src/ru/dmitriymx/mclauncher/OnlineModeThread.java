		...
		try{
			for(int i = 0; i < Config.MINECRAFT_JARS.length; i++){
				if( ((!check[0]) && (i == 0)) || ((!check[1]) && (i > 0)) ){
					...
					http.sendRequestHeader();
					DResponse resp = http.getResponse();
					FileOutputStream fos = new FileOutputStream(new File(Config.MINECRAFT_BINPATH, Config.MINECRAFT_JARS[i]));
			...
			if(!check[1]){
				...
				while (zipFileEntries.hasMoreElements()){
					...
					File destinationFilePath = new File(Config.MINECRAFT_BINPATH + File.separator + "natives" + File.separator + entry.getName());
	...
	private boolean[] CheckSumClient(boolean[] check){
		...
		if(check[1]){
			...
			for(int i = 1; i < Config.MINECRAFT_JARS.length; i++){
				try{
					FileInputStream fis = new FileInputStream(new File(Config.MINECRAFT_BINPATH, Config.MINECRAFT_JARS[i]));
